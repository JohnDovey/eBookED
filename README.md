...
| Project | Purpose |
|---|---|
| `AvaloniaEdit` | **Unused, pending removal.** Was a vendored copy of [AvaloniaEdit](https://github.com/AvaloniaUI/AvaloniaEdit), kept in-repo only to avoid an assembly collision with `Markdown.Avalonia`'s syntax-highlighting extension. `eBookEditor.App` now references the real `Avalonia.AvaloniaEdit` NuGet package directly, and this vendored copy is slated for deletion. |
