...
Right-click the editor → **Insert Image…** opens a file picker rooted at the project's `images/` folder (created if missing); picking a file from anywhere else copies it into `images/` first (auto-renamed on a name collision), so a chapter's images always end up self-contained in the project. The image is inserted as a nested custom container — a classless outer `::::` wrapping the image and an inner `::: {.caption}` block for the caption text, e.g.:
...
This is deliberately not a Markdown table: attaching a trailing `{.class}` attribute block immediately after a pipe table makes Markdig fail to recognize the table at all and fall back to rendering it as literal pipe-character text (verified directly against the pipeline — not documented anywhere) — nested containers are the only reliable way to get an independently-styled caption. Nesting requires the outer fence to use more colons than the inner one, or Markdig leaves a stray empty `<div></div>` behind (also undocumented, also verified directly).

### Preview rendering vs. export rendering

The Open Preview window uses `Markdown.Avalonia`, a separate, independent Markdown parser (not Markdig) with no support for custom containers or special attribute blocks at all — feeding either to it verbatim doesn't just leave them unstyled, it shows visibly broken output (a raw `:::` fence renders as a literal code block; a `{#id .class}` block shows as literal trailing text on a heading). `PreviewMarkdownSanitizer` (`eBookEditor.Markdown`) strips both before handing content to the preview, so "Apply Style" text and inserted images with captions show as clean plain text/images there instead — readable, but not styled the way the EPUB actually renders it, since Preview has no CSS engine of its own either way. This only affects what's displayed in the preview window; the saved chapter file and every export format are unaffected.
...
- The preview window's "scroll to follow the cursor" sync is proportional (cursor's line number ÷ total lines, mapped onto the preview's scroll range), not an exact mapping from a source line to the rendered block it produced — Markdown.Avalonia doesn't expose a way to resolve a rendered element back to its originating Markdown source position. It lands you in roughly the right place, not the exact line.
