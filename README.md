...
- Footnotes (inserted via **Insert Footnote…**) get a real clickable jump/return link in EPUB, styled via the built-in `.footnote-ref`/`.footnotes`/`.footnote-back-ref` CSS rules. PDF and Word have no dedicated footnote handling yet — the reference and its note render as ordinary superscript text and a trailing list at the end of the chapter (structurally in the right place, but not a page-jump link, and not, for Word, a native `FootnotesPart`/footnote-pane entry); see the doc comments atop `HtmlToPdfRenderer`/`HtmlToDocxConverter`.
